...
def main():
    ...
    for line in code_lines:
        ...
        data = re.match(r"""^(\s*)driver\.find_element_by_id\(\"(\S+)\"\)""" r"""\.click\(\)\s*$""", line,)
        if data:
            whitespace = data.group(1)
        ...
        data = re.match(r"""^(\s*)driver\.find_element_by_id\(\"(\S+)\"\)""" r"""\.submit\(\)\s*$""", line,)
        if data:
            whitespace = data.group(1)
            selector = "#%s" % data.group(2).replace("#", "\\#")
            selector = selector.replace("[", "\\[").replace("]", "\\]")
        ...
        data = re.match(
            r"""^(\s*)driver\.find_element_by_id\(\"(\S+)\"\)""" r"""\.send_keys\(\"([\S\s]+)\"\)\s*$""",
            line,
        )
        if data:
            whitespace = data.group(1)
            ...
            if "\\[" in selector or "\\]" in selector or "\\." in selector:
                raw = "r"
            text = data.group(3)
            command = """%sself.update_text(%s'%s', '%s')""" % (whitespace, raw, selector, text,)
            seleniumbase_lines.append(command)
            continue

        # Handle .find_element_by_id() + .send_keys(Keys.<KEY>)
        data = re.match(
            r"""^(\s*)driver\.find_element_by_id\(\"(\S+)\"\)""" r"""\.send_keys\(Keys\.([\S]+)\)\s*$""",
            line,
        )
        if data:
            uses_keys = True
        ...
        data = re.match(r"""^(\s*)driver\.find_element_by_name\(\"(\S+)\"\)""" r"""\.click\(\)\s*$""", line,)
        if data:
            whitespace = data.group(1)
        ...
        data = re.match(r"""^(\s*)driver\.find_element_by_name\(\"(\S+)\"\)""" r"""\.submit\(\)\s*$""", line,)
        if data:
            whitespace = data.group(1)
            selector = '[name="%s"]' % data.group(2)
        ...
        data = re.match(
            r"""^(\s*)driver\.find_element_by_name\(\"(\S+)\"\)""" r"""\.send_keys\(\"([\S\s]+)\"\)\s*$""",
            line,
        )
        if data:
            whitespace = data.group(1)
            selector = '[name="%s"]' % data.group(2)
            text = data.group(3)
            command = """%sself.update_text('%s', '%s')""" % (whitespace, selector, text,)
            seleniumbase_lines.append(command)
            continue

        # Handle .find_element_by_name() + .send_keys(Keys.<KEY>)
        data = re.match(
            r"""^(\s*)driver\.find_element_by_name\(\"(\S+)\"\)""" r"""\.send_keys\(Keys\.([\S]+)\)\s*$""",
            line,
        )
        if data:
            uses_keys = True
        ...
        data = re.match(
            r"""^(\s*)driver\.find_element_by_css_selector\(\"([\S\s]+)\"\)""" r"""\.click\(\)\s*$""", line,
        )
        if data:
            whitespace = data.group(1)
        ...
        data = re.match(
            r"""^(\s*)driver\.find_element_by_css_selector\(\"([\S\s]+)\"\)""" r"""\.submit\(\)\s*$""", line,
        )
        if data:
            whitespace = data.group(1)
            selector = "%s" % data.group(2)
        ...
        data = re.match(
            r"""^(\s*)driver\.find_element_by_xpath\(u?\"([\S\s]+)\"\)""" r"""\.click\(\)\s*$""", line,
        )
        if data:
            whitespace = data.group(1)
            ...
            if ".//*[normalize-space(text())" in xpath and ("normalize-space(.)='" in xpath):
                x_match = re.match(
                    r"""^[\S\s]+normalize-""" r"""space\(\.\)=\'([\S\s]+)\'\]\)[\S\s]+""", xpath,
                )
                if x_match:
        ...
        data = re.match(
            r"""^(\s*)driver\.find_element_by_xpath\(u?\"([\S\s]+)\"\)""" r"""\.submit\(\)\s*$""", line,
        )
        if data:
            whitespace = data.group(1)
            xpath = "%s" % data.group(2)
        ...
        data = re.match(
            r"""^(\s*)driver\.find_element_by_link_text\(u?\"([\S\s]+)\"\)""" r"""\.click\(\)\s*$""", line,
        )
        if data:
            whitespace = data.group(1)
        ...
        data = re.match(
            r"""^(\s*)([\S\s]*)self\.is_element_present\(By.LINK_TEXT, """ r"""u?\"([\S\s]+)\"\)([\S\s]*)$""",
            line,
        )
        if data:
            whitespace = data.group(1)
        ...
        data = re.match(
            r"""^(\s*)([\S\s]*)self\.is_element_present\(By.NAME, """ r"""u?\"([\S\s]+)\"\)([\S\s]*)$""",
            line,
        )
        if data:
            whitespace = data.group(1)
        ...
        data = re.match(
            r"""^(\s*)([\S\s]*)self\.is_element_present\(By.ID, """ r"""u?\"([\S\s]+)\"\)([\S\s]*)$""", line,
        )
        if data:
            whitespace = data.group(1)
            pre = data.group(2)
        ...
        data = re.match(
            r"""^(\s*)([\S\s]*)self\.is_element_present\(By.CLASS, """ r"""u?\"([\S\s]+)\"\)([\S\s]*)$""",
            line,
        )
        if data:
            whitespace = data.group(1)
        ...
        data = re.match(
            r"""^(\s*)([\S\s]*)self\.is_element_present\(By.XPATH, """ r"""u?\"([\S\s]+)\"\)([\S\s]*)$""",
            line,
        )
        if data:
            whitespace = data.group(1)
    ...
    for line in lines:
        ...
        if in_inefficient_wait:
            data = re.match(r"""^\s*if self.is_element_present\("([\S\s]+)"\)""" r""": break\s*$""", line,)
            if data:
            ...
            data = re.match(r"""^\s*if self.is_link_text_present""" r"""\("([\S\s]+)"\): break\s*$""", line,)
            if data:
                uni = ""
                if '(u"' in line:
                    uni = "u"
    ...
    for line_num in range(len(lines)):
        data = re.match(
            r"""^\s*self.wait_for_element""" r"""\((["|'])([\S\s]+)(["|'])\)""" r"""\s*$""", lines[line_num],
        )
        if data:
            ...
            if int(line_num) < num_lines - 1:
                regex_string = r"""^\s*self.click\(["|']""" "" + selector + r"""["|']\)\s*$"""
                data2 = re.match(regex_string, lines[line_num + 1])
                if data2:
                    continue
                regex_string = (
                    r"""^\s*self.update_text\(["|']""" "" + selector + "" "" + r"""["|'], [\S\s]+\)\s*$"""
                )
                data2 = re.match(regex_string, lines[line_num + 1])
                if data2:
                    continue
    ...
    for line_num in range(len(lines)):
        data = re.match(
            r"""^\s*self.wait_for_link_text""" r"""\((["|'])([\S\s]+)(["|'])\)""" r"""\s*$""",
            lines[line_num],
        )
        if data:
            # quote_type = data.group(1)
            ...
            if int(line_num) < num_lines - 2:
                regex_string = r"""^\s*self.click\(["|']link=""" "" + link_text + r"""["|']\)\s*$"""
                data2 = re.match(regex_string, lines[line_num + 1])
                if data2:
                    continue
        seleniumbase_lines.append(lines[line_num])
