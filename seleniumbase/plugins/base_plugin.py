...
class Base(Plugin):
    ...
    def options(self, parser, env):
        ...
        parser.add_option(
            "--data", dest="data", default=None, help="Extra data to pass to tests from the command line.",
        )
        parser.add_option(
            "--var1", dest="var1", default=None, help="Extra data to pass to tests from the command line.",
        )
        parser.add_option(
            "--var2", dest="var2", default=None, help="Extra data to pass to tests from the command line.",
        )
        parser.add_option(
            "--var3", dest="var3", default=None, help="Extra data to pass to tests from the command line.",
        )
        parser.add_option(
            "--settings_file",
            "--settings-file",
    ...
    def __log_all_options_if_none_specified(self, test):
        ...
        if (
            ...
            log_helper.log_test_failure_data(test, test_logpath, test.driver, test.browser)
    ...
    def addError(self, test, err, capt=None):
        ...
        """
        if err[0] == errors.BlockedTest or (err[0] == errors.SkipTest) or (err[0] == errors.DeprecatedTest):
            print(
                err[1]
                .__str__()
                .split(
