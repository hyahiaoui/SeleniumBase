...
from seleniumbase.core import capabilities_parser, download_helper, proxy_helper
...
def _set_chrome_options(
    ...
    if proxy_string:
        if proxy_auth:
            chrome_options = _add_chrome_proxy_extension(chrome_options, proxy_string, proxy_user, proxy_pass)
...
def _create_firefox_profile(downloads_path, proxy_string, user_agent, disable_csp):
    ...
    profile.set_preference("datareporting.healthreport.logging.consoleEnabled", False)
    ...
    profile.set_preference("datareporting.policy.dataSubmissionPolicyAccepted", False)
...
def get_remote_driver(
    ...
    if browser_name == constants.Browser.GOOGLE_CHROME:
        ...
        return webdriver.Remote(command_executor=address, desired_capabilities=capabilities, keep_alive=True)
    elif browser_name == constants.Browser.FIREFOX:
        try:
            # Use Geckodriver for Firefox if it's on the PATH
            profile = _create_firefox_profile(downloads_path, proxy_string, user_agent, disable_csp)
        ...
        with warnings.catch_warnings():
            ...
            return webdriver.Remote(
                command_executor=address, desired_capabilities=capabilities, keep_alive=True,
            )
        ...
        return webdriver.Remote(command_executor=address, desired_capabilities=desired_caps, keep_alive=True)
        ...
        try:
            try:
                ...
                firefox_driver = webdriver.Firefox(
                    firefox_profile=profile, capabilities=firefox_capabilities, options=options,
                )
            except Exception:
                profile = _create_firefox_profile(downloads_path, proxy_string, user_agent, disable_csp)
                firefox_capabilities = DesiredCapabilities.FIREFOX.copy()
                firefox_driver = webdriver.Firefox(firefox_profile=profile, capabilities=firefox_capabilities)
        ...
        try:
            ...
            if LOCAL_EDGEDRIVER and os.path.exists(LOCAL_EDGEDRIVER):
                ...
                if not ("-n" in sys.argv or "-n=" in args or args == "-c"):
                    ...
                    sys.argv = sys_args  # Put back the original sys args
            return webdriver.Chrome(executable_path=LOCAL_EDGEDRIVER, options=chrome_options)
        except Exception as e:
            if headless:
                raise
            if LOCAL_EDGEDRIVER and os.path.exists(LOCAL_EDGEDRIVER):
