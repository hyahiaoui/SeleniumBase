...
def play_shepherd_tour(driver, tour_steps, msg_dur, name=None, interval=0):
    ...
    if len(tour_steps[name]) > 1:
        try:
            selector = re.search(r"[\S\s]+{element: '([\S\s]+)', on: [\S\s]+", tour_steps[name][1]).group(1)
            selector = selector.replace("\\", "")
            page_actions.wait_for_element_present(
                driver, selector, by=By.CSS_SELECTOR, timeout=settings.SMALL_TIMEOUT
            )
        except Exception:
    ...
    if len(tour_steps[name]) > 1:
        try:
            if "element: " in tour_steps[name][1]:
                selector = re.search(
                    r"[\S\s]+element: '([\S\s]+)',[\S\s]+title: '", tour_steps[name][1]
                ).group(1)
    ...
    while tour_on:
        try:
            ...
            if browser != "firefox":
                ...
                page_actions.wait_for_element_present(driver, ".tour-tour", by=By.CSS_SELECTOR, timeout=0.4)
                result = False
        except Exception:
            tour_on = False
            result = None
    ...
    if len(tour_steps[name]) > 1:
        try:
            if "target: " in tour_steps[name][1]:
                selector = re.search(
                    r"[\S\s]+target: '([\S\s]+)',[\S\s]+title: '", tour_steps[name][1]
                ).group(1)
                selector = selector.replace("\\", "").replace(":first", "")
                page_actions.wait_for_element_present(
                    driver, selector, by=By.CSS_SELECTOR, timeout=settings.SMALL_TIMEOUT
    ...
    while tour_on:
        try:
            ...
            if browser != "firefox":
                ...
                page_actions.wait_for_element_present(
                    driver, ".hopscotch-bubble", by=By.CSS_SELECTOR, timeout=0.4
                )
                result = False
        except Exception:
            tour_on = False
...
def play_introjs_tour(driver, tour_steps, browser, msg_dur, name=None, interval=0):
    ...
    if len(tour_steps[name]) > 1:
        try:
            if "element: " in tour_steps[name][1]:
                selector = re.search(
                    r"[\S\s]+element: '([\S\s]+)',[\S\s]+intro: '", tour_steps[name][1]
                ).group(1)
                selector = selector.replace("\\", "")
                page_actions.wait_for_element_present(
                    driver, selector, by=By.CSS_SELECTOR, timeout=settings.SMALL_TIMEOUT
