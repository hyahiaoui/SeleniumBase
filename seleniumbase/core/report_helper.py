...
def process_failures(test, test_count, browser_type, duration):
    ...
    page_actions.save_test_failure_data(test.driver, bad_page_data, browser_type, folder=LATEST_REPORT_DIR)
...
def clear_out_old_report_logs(archive_past_runs=True, get_log_folder=False):
    ...
    if archive_past_runs:
        ...
        archive_dir = "%s/../%s/report_%s" % (file_path, ARCHIVE_DIR, archive_timestamp)
...
def build_report(report_log_path, page_results_list, successes, failures, browser_type, show_report):
    ...
    log_link_shown = "../%s%s/" % (ARCHIVE_DIR, web_log_path.split(ARCHIVE_DIR)[1])
    ...
    for line in page_results_list:
        ...
        if line[1] == '"FAILED!"':
            ...
            line = (
                '<a href="%s">%s</a>' % ("file://" + report_log_path + "/" + line[2], line[2])
                + """
                &nbsp;&nbsp;
                """
    ...
    report_html = "<html><head>%s</head><body>%s</body></html>" % (style, table_view)
    ...
    if show_report:
        ...
        if browser_type == "firefox":
            if LOCAL_GECKODRIVER and os.path.exists(LOCAL_GECKODRIVER):
                browser = webdriver.Firefox(firefox_profile=profile, executable_path=LOCAL_GECKODRIVER)
            else:
            ...
            if LOCAL_CHROMEDRIVER and os.path.exists(LOCAL_EDGEDRIVER):
                browser = webdriver.Chrome(executable_path=LOCAL_EDGEDRIVER, options=edge_options)
            else:
            ...
            if LOCAL_CHROMEDRIVER and os.path.exists(LOCAL_CHROMEDRIVER):
                browser = webdriver.Chrome(executable_path=LOCAL_CHROMEDRIVER, options=chrome_options)
            else:
