...
from seleniumbase.virtual_display.unicodeutil import split_command, unidecode, uniencode
import logging
import os.path
import platform
import signal
...
class EasyProcess(object):
    ...
    def __init__(
        self, cmd, ubuntu_package=None, url=None, cwd=None, use_temp_files=True, env=None,
    ):
    ...
    def check(self, return_code=0):
        ...
        ok = ret == return_code
        if not ok:
            raise EasyProcessError(self, "check error, return code is not {0}!".format(return_code))
        return self
